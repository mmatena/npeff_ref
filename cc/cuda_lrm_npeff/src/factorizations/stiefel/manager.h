...
protected:
    ...
    void log_loss(const std::string& prefix, int64_t step, std::vector<float>& losses) {
        ...
        // std::cout << prefix << " step " << step + 1 << ": " << std::fixed << std::setprecision(20) <<  loss << " [" << elapsed_ms / (double) config.log_loss_frequency << " ms/step]\n";
    ...
    void G_update_step(bool recompute_AG_GG, float learning_rate_G) {
        ...
        call_on_workers_then_join(
            &DeviceWorker<IndT>::compute_local_G_update_intermediates_after_AG_GG_all_reduces_async);

        all_reduce_GD_DD_async();

        call_on_workers_then_join(
            &DeviceWorker<IndT>::update_local_G_after_GD_DD_all_reduces_async, learning_rate_G);
    ...
    void all_reduce_GD_DD_async() {
        NCCL_CALL(ncclGroupStart());
        for(auto& worker : workers) {
            worker->nccl_all_reduce_GD_DD();
        }
        NCCL_CALL(ncclGroupEnd());
    }
    ...
    // void call_nccl_group_on_workers(void (DeviceWorker<IndT>::*method)(Args...), Args... args) {
    //     NCCL_CALL(ncclGroupStart());
    //     for(auto& worker : workers) {
    //         method(args...);
    //     }
